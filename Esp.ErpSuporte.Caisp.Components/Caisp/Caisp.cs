...
using System.Linq;
using Benner.Corporativo.Definicoes.Contratos;
using System.Globalization;
using Esp.Erpsuporte.Caisp.Business.Interfaces.Caisp;
using Benner.Corporativo.Wes.WebApp.aga.a.Financeiro.CodigoBarras;
...
namespace Esp.ErpSuporte.Caisp.Components.Caisp
{
    public class CaispComponente : BusinessComponent<CaispComponente>, ICaisp
    {
        ...
        public List<ProgramacaoModel> buscarProgramacao(ProgramacaoBuscarModel request)
        {
            ...
            //Programação total/ filtro * periodo //analise de dois de periodos
            //    programado * quantidade de dias selecionados
            var registros = query.Execute();

            foreach (EntityBase registro in registros)
            {

                int ProgramadoTotal = 0;
                TimeSpan diferenca = TimeSpan.Zero;
                DateTime dataInicioProgramado = Convert.ToDateTime(registro.Fields["DATAINICIO"]);
                DateTime dataFimProgramado = Convert.ToDateTime(registro.Fields["DATAFIM"]);
                TimeSpan diferencaProgramado= dataFimProgramado - dataInicioProgramado;
                int dias = diferencaProgramado.Days + 1;


                DateTime dataInicioFiltro = DateTime.ParseExact(request.Inicio, "dd/MM/yyyy", CultureInfo.InvariantCulture);
                DateTime dataFimFiltro = DateTime.ParseExact(request.Fim, "dd/MM/yyyy", CultureInfo.InvariantCulture);
                if(dataInicioFiltro >= dataInicioProgramado && dataFimFiltro <= dataFimProgramado)
                {
                    diferenca = dataFimFiltro - dataInicioFiltro;
                    ProgramadoTotal = Convert.ToInt32(registro.Fields["QUANTIDADEPROGRAMADA"])/dias * (diferenca.Days + 1);
                }
                else if (dataInicioFiltro <= dataInicioProgramado && dataFimFiltro >= dataFimProgramado)
                {
                    ProgramadoTotal = Convert.ToInt32(registro.Fields["QUANTIDADEPROGRAMADA"]);
                }
                else if (dataInicioFiltro >= dataInicioProgramado && dataFimFiltro >= dataFimProgramado)
                {
                    diferenca = dataFimProgramado - dataInicioFiltro;
                    ProgramadoTotal = Convert.ToInt32(registro.Fields["QUANTIDADEPROGRAMADA"]) / dias * (diferenca.Days + 1);
                }
                else if (dataInicioFiltro <= dataInicioProgramado && dataFimFiltro <= dataFimProgramado)
                {
                    diferenca = dataFimFiltro - dataInicioProgramado;
                    ProgramadoTotal = Convert.ToInt32(registro.Fields["QUANTIDADEPROGRAMADA"]) / dias * (diferenca.Days + 1);
                }
        ...
        public string ProcessarAnalise(ProcessarAnaliseModel request)
        {
            ...
            //BusinessTask.Factory.NewComponentTask<IProcessarAnalise>()
            //            .WithDescription("Analise Do Cooperado")
            //            .WithNotification()
        ...
        public string CarregarFornecedores(RequestCarregarFornecerdor request)
        {
            ...
            else
            {
                ...
                foreach (var registro in registros)
                {
                    Query VerificarRegistros = new Query($@"SELECT C.HANDLE,
                                                                        C.COTA,
                                                                        C.QUANTIDADECOTA,
                                                                        C.QUANTIDADEPROGRAMADA,
                                                                        D.PRODUTO,
                                                                        ...
                                                                        JOIN PD_PRODUTOS E ON D.PRODUTO = E.HANDLE
                                                                        JOIN K_CM_PROGRAMADOS P ON D.PROGRAMADOS = P.HANDLE
                                                                        WHERE C.FORNECEDOR = :PRODUTOR AND D.PRODUTO = :PRODUTOMATRIZ AND C.PROGRAMADOPRODUTO = :HANDLE");
                    VerificarRegistros.Parameters.Add("PRODUTOR", Convert.ToInt32(registro.Fields["PRODUTOR"]));
                    VerificarRegistros.Parameters.Add("PRODUTOMATRIZ", Convert.ToInt32(registro.Fields["PRODUTOMATRIZ"]));
                    VerificarRegistros.Parameters.Add("HANDLE", request.Handle);
                    var verificar = VerificarRegistros.Execute();
                    if (verificar.Count == 0)
                    {
                    ...
                    else
                    {
                        ...
                        foreach(var registro1 in verificar)
                        {
                            ...
                            Soma += Convert.ToDouble(registro1.Fields["COTA"]);
                            Contador +=1;
                        }
                        EntityBase ProgramadoFornecedores = Entity.Get(EntityDefinition.GetByName("K_CM_PROGRAMADOFORNECEDORES"), new Criteria($"A.HANDLE = :HANDLE", new Parameter("HANDLE", Handle)), GetMode.Edit);
                        ProgramadoFornecedores.Fields["COTA"] = Soma / Contador;
                        ProgramadoFornecedores.Fields["QUANTIDADECOTA"] = ((Soma / Contador) / 100) * request.Quantidade;
                        ProgramadoFornecedores.Save();
        ...
        public void AnaliseFaturamento()
        {
            Query qFaturamento = new Query(@"SELECT FN_DOCUMENTOS.DATAEMISSAO,
                                            ...
                                            ");
            qFaturamento.Parameters.Add("DATAINICO", "01/04/2021 05:00:00");
            qFaturamento.Parameters.Add("DATAFIM", "01/05/2021 05:00:00");
            ...
            Query qBonificacao = new Query(@"SELECT FN_DOCUMENTOS.DATAEMISSAO,
                                     ...
                                     ORDER BY FN_DOCUMENTOS.DATAEMISSAO,
                                           FN_DOCUMENTOS.DOCUMENTODIGITADO ");
            qBonificacao.Parameters.Add("DATAINICO", "01/04/2021 05:00:00");
            qBonificacao.Parameters.Add("DATAFIM", "01/05/2021 05:00:00");
            ...
            Query qDevolucao = new Query(@"SELECT FN_DOCUMENTOS.DATAEMISSAO,
                                        ...
                                        ");
            DateTime date = new DateTime(2021, 5, 01); // Exemplo de data
            qDevolucao.Parameters.Add("DATAINICO", "01/04/2021");
            ...
            var query = from d in DadosAnalise
                        ...
                        select new
                        {
                            ...
                            FaturamentoBruto = g.Sum(x => x.FaturamentoBruto),
                            Bonificacao = g.Sum(x => x.Bonificacao),
                            Devolucao = g.Sum(x => x.Devolucao),
                            DF = g.Where(x => x.DF > 0).DefaultIfEmpty().Average(x => x.DF),
                            ICMS = g.Where(x => x.ICMS > 0).DefaultIfEmpty().Average(x => x.ICMS),
                            ReducaoICMS = g.Where(x => x.ReducaoICMS > 0).DefaultIfEmpty().Average(x => x.ReducaoICMS),
                            PorcentagemICMS = g.Where(x => x.PorcentagemICMS > 0).DefaultIfEmpty().Average(x => x.PorcentagemICMS),
                            PIS = g.Where(x => x.PIS > 0).DefaultIfEmpty().Average(x => x.PIS),
                            COFINS = g.Where(x => x.COFINS > 0).DefaultIfEmpty().Average(x => x.COFINS),
